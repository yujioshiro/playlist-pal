...
export async function getInitialSongsFromOpenAiChatCompletion(prompt: string): Promise<{ result: string }> {
    try {
        ...
        const completion = await openai.createChatCompletion({
            model: "gpt-4",
            messages: [{role:'user', content:`You are the world's best music curator. give me 7 songs that sound like ${prompt}.Results in JSON,omit all other text.Example: [{"artist":"artist name","song":"song name"},{"artist":"artist name","song":"song name"},{"artist":"artist name","song":"song name"}]`}],
            temperature: 0.7,
            max_tokens: 750,
        })
